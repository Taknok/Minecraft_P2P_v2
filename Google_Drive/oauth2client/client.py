...
HAS_CRYPTO = False
try:
  from Google_Drive.oauth2client import crypt
  HAS_CRYPTO = True
...
class Credentials(object):
  ...
  def new_from_json(cls, s):
    ...
    try:
      ...
      module = module.replace('Google_Drive.apiclient', '')
      m = __import__(module)
...
class Storage(object):
  ...
  def locked_get(self):
    ...
    Returns:
      Google_Drive.oauth2client.client.Credentials
    """
    _abstract()
...
if HAS_CRYPTO:
  # PyOpenSSL and PyCrypto are not prerequisites for Google_Drive.oauth2client, so if it is
  ...
  def verify_id_token(id_token, audience, http=None,
    ...
    Raises:
      Google_Drive.oauth2client.crypt.AppIdentityError if the JWT fails to verify.
    """
    if http is None:
      http = _cached_http
